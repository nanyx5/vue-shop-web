      ...
      <el-form :model="addForm" :rules="addFormrules" ref="addFormRef"
        ...
        <el-tabs :tab-position="tabPosition" v-model="activeIndex"
          ...
          <el-tab-pane label="商品内容" name="4">
            <quill-editor v-model="addForm.good_introduce" ref="myQuillEditor">
            </quill-editor>
  ...
  data() {
    return {
      ...
      addFormrules: {
        ...
        goods_cat: [{ required: true, message: '', trigger: 'blur' }],
      },
    }
  },
  created() {
  ...
  methods: {
    add() {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        this.$message.error('添加商品失败失败')
        this.$message.success('添加商品成功')
        this.$router.push('/goods')
      })
    },
    handleSuccess(response) {
      const picInfo = { pcis: response.data.tmp_path }
